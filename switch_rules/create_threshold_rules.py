...
def add_switch_rules(pod_num, file_name, which_row):
    """
    Desc: 根据dscp_remark_rate二维array来设置不同位置的threshold
    """
    
    threshold = rt.get_config(file_name, which_row = which_row, pod_num = pod_num)
    ...
    for src in range(1, pod_num + 1):
        for dst in range(1, pod_num + 1):
            if dst != src:
                ...
                # 2跳routing
                rules_str += add_group(f's{src}', dst, src, dst, pod_num) + '\n'
                rules_str += add_normal_flow(
                    switch_name = f's{src}',
                    table_id = 1,
                    match_dict = {
...
if __name__ == "__main__":
    pod_num = 4
    res = add_switch_rules(pod_num, f'../traffic/{pod_num}pod_threshold.csv', 0)
    with open(f'./{pod_num}pod_threshold_rules.sh', 'w') as f:
        f.write(res)